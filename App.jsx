...
const PAT = {
  ...
  isbn: /ISBN[:\s]+[\d\-Xx]+/i,
  issn: /ISSN[:\s]+[\d\-Xx]+/i,
  acessoEm: /Acesso em:\s*\d+\s+[a-zçãáéíóú.]+\s+\d{4}/i,
...
function extrairComponentes(s) {
  ...
  if (PAT.anoIncerto.test(s)) {
    ...
    const anos = s.match(/\b(1[5-9]\d{2}|20\d{2})\b/g);
    if (anos) {
  ...
  // Acesso em
  if (PAT.acessoEm.test(s)) {
    c.acesso = { valor: s.match(PAT.acessoEm)[0], status: "ok" };
  }
  ...
  // Periódico (artigo) — heurística: trecho entre fim do título e antes de "v." ou "p.", ou padrão palavra-chave
  if (PAT.volume.test(s) || PAT.paginas.test(s) || PAT.periodico.test(s)) {
    // Tenta pegar o texto entre o último ponto antes do "v."/"p." e o próprio "v."/"p."
    ...
    if (periMatch) {
      ...
      // Fallback: procurar palavras-chave conhecidas
      const fallback = s.match(/\.\s+([A-Z][^.,]*?(?:Revista|Journal|Cadernos|Bolet[ií]m|Arquivos|Anais|Estudos|An[áa]lise|Ci[êe]ncia|Acta|Folha|Educação|Sociedade)[^,.]*?)\s*,/i);
      if (fallback) {
...
function validarCapitulo(s, c, estrito) {
  const problemas = [], acertos = [], alertas = [];
  if (!PAT.inPattern.test(s)) {
    problemas.push("Falta 'In:' antes do livro principal (NBR 6023:2018, 7.3).");
  } else {
...
function construirCorrecao(s, c, tipo) {
  ...
  // 2. Tratamento para material ONLINE (site / institucional online / artigo online)
...
function analisarReferencia(texto, tipoHint = "auto", estrito = false) {
  ...
  const sugestaoCorrigida = construirCorrecao(s, componentes, tipoDetectado);
  ...
  if (tipoDetectado === "artigo" || tipoDetectado === "artigo-online") regrasAplicadas.push(REGRAS_NORMA.artigo);
...
function ModuleValidadorABNT() {
  ...
  return (
    <div>
      ...
      {/* ───── V2 HÍBRIDA: BOTÃO REFINAR COM IA + PAINEL DA IA ───── */}
      {resultado && (
        <Card style={{ marginTop: 18, marginBottom: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            <div style={{ flex: 1, minWidth: 200 }}>
              <SubTitle color={t.purple}>🤖 Análise refinada por IA (opcional)</SubTitle>
              <div style={{ color: t.textMuted, fontSize: 12, lineHeight: 1.5 }}>
                A análise local verifica padrões formais. A análise com IA melhora a interpretação,
                mas ainda exige conferência da fonte original em casos ambíguos.
              </div>
              <div
                style={{
                  marginTop: 8,
                  ...
                  background: t.surfaceAlt,
                  border: `1px solid ${t.border}`,
                  borderLeft: `3px solid ${t.purple}66`,
                  borderRadius: 6,
                  fontSize: 11,
                  color: t.textMuted,
                  lineHeight: 1.5,
              ...
              >
                <b style={{ color: t.text }}>Transparência:</b> a análise com IA <b>não consulta os PDFs da ABNT em tempo real</b> neste deploy. Ela interpreta a referência com base nas regras codificadas no sistema, na análise local e no modelo de linguagem.
              </div>
            </div>
            <button
              ...
              style={{
                padding: "10px 18px",
                background: iaCarregando ? t.surfaceAlt : `linear-gradient(135deg, ${t.purple}, ${t.pink})`,
                color: iaCarregando ? t.textMuted : "#fff",
                border: "none",
                borderRadius: 10,
                fontWeight: 700,
                fontSize: 13,
                cursor: iaCarregando ? "wait" : "pointer",
                boxShadow: iaCarregando ? "none" : `0 8px 24px -8px ${t.purple}88`,
                display: "inline-flex",
                alignItems: "center",
                gap: 6,
                whiteSpace: "nowrap",
            ...
            >
              {iaCarregando ? "⏳ Analisando..." : iaResultado ? "🔄 Analisar de novo" : "✨ Refinar com IA"}
            </button>
          </div>
          ...
          {iaCarregando && (
            ...
            >
              ...
              <div>
                Enviando para análise serverless... isso pode levar alguns segundos.<br/>
                <span style={{ color: t.textMuted, fontSize: 11 }}>
                  A análise local acima continua válida — esta é uma camada adicional.
                </span>
              </div>
            </div>
          )}
          ...
          {iaErro && !iaCarregando && (
            ...
            >
              <div style={{ fontWeight: 700, marginBottom: 4, color: t.yellow }}>
                ⚠️ IA indisponível
              </div>
              <div>{iaErro.msg}</div>
              <div style={{ marginTop: 6, fontSize: 11, color: t.textMuted }}>
                A análise local acima continua válida e disponível.
              </div>
            </div>
          )}

          {/* Estado: sucesso */}
          {iaResultado && !iaCarregando && (
            <div style={{ marginTop: 16, display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              >
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>
                  ...
                  {typeof iaResultado.pontuacao === "number" && (
                    <span
                      style={{
                        padding: "3px 8px",
                        background: t.surfaceAlt,
                    ...
                    >
                      📊 {iaResultado.pontuacao}/100
                    </span>
                  )}
                  {typeof iaResultado.confianca === "number" && (
                    <span
                      style={{
                        padding: "3px 8px",
                        background: t.surfaceAlt,
                    ...
                    >
                      Confiança: <b>{iaResultado.confianca}%</b>
                    </span>
                  )}
                </div>
                {iaResultado.resumo && (
                  <div style={{ color: t.text, fontSize: 13.5, lineHeight: 1.6, marginTop: 4 }}>
                    {iaResultado.resumo}
                  </div>
                )}
              </div>

              {/* Validação formal vs bibliográfica */}
              {(iaResultado.validacaoFormal?.length > 0 || iaResultado.validacaoBibliografica?.length > 0) && (
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  {iaResultado.validacaoFormal?.length > 0 && (
                    <div
                      style={{
                        padding: "12px 14px",
                    ...
                    >
                      <div style={{ fontSize: 11, color: t.cyan, fontWeight: 800, marginBottom: 8, letterSpacing: "0.05em" }}>
                        📐 VALIDAÇÃO FORMAL
                      </div>
                      <ul style={{ margin: 0, paddingLeft: 18, color: t.text, fontSize: 12.5, lineHeight: 1.6 }}>
                        {iaResultado.validacaoFormal.map((v, i) => (
                          <li key={i}>{v}</li>
                        ))}
                      </ul>
                    </div>
                  )}
                  {iaResultado.validacaoBibliografica?.length > 0 && (
                    <div
                      style={{
                        padding: "12px 14px",
                    ...
                    >
                      <div style={{ fontSize: 11, color: t.orange, fontWeight: 800, marginBottom: 8, letterSpacing: "0.05em" }}>
                        📚 VALIDAÇÃO BIBLIOGRÁFICA
                      </div>
                      <ul style={{ margin: 0, paddingLeft: 18, color: t.text, fontSize: 12.5, lineHeight: 1.6 }}>
                        {iaResultado.validacaoBibliografica.map((v, i) => (
                          <li key={i}>{v}</li>
                        ))}
                      </ul>
                    </div>
              ...
              {/* Acertos / Problemas / Alertas */}
              {(iaResultado.acertos?.length > 0 || iaResultado.problemas?.length > 0 || iaResultado.alertas?.length > 0) && (
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {iaResultado.problemas?.length > 0 && (
                    <div
                      style={{
                        padding: "10px 12px",
                        background: t.redSoft,
                    ...
                    >
                      <div style={{ fontSize: 11, color: t.red, fontWeight: 800, marginBottom: 6 }}>
                        ❌ PROBLEMAS ({iaResultado.problemas.length})
                      </div>
                      <ul style={{ margin: 0, paddingLeft: 18, color: t.text, fontSize: 12.5, lineHeight: 1.6 }}>
                        {iaResultado.problemas.map((p, i) => <li key={i}>{p}</li>)}
                      </ul>
                    </div>
                  )}
                  {iaResultado.alertas?.length > 0 && (
                    <div
                      style={{
                        padding: "10px 12px",
                        background: t.yellowSoft,
                    ...
                    >
                      <div style={{ fontSize: 11, color: t.yellow, fontWeight: 800, marginBottom: 6 }}>
                        ⚠️ ALERTAS ({iaResultado.alertas.length})
                      </div>
                      <ul style={{ margin: 0, paddingLeft: 18, color: t.text, fontSize: 12.5, lineHeight: 1.6 }}>
                        {iaResultado.alertas.map((a, i) => <li key={i}>{a}</li>)}
                      </ul>
                    </div>
                  )}
                  {iaResultado.acertos?.length > 0 && (
                    <div
                      style={{
                        padding: "10px 12px",
                        background: t.greenSoft,
                    ...
                    >
                      <div style={{ fontSize: 11, color: t.green, fontWeight: 800, marginBottom: 6 }}>
                        ✅ ACERTOS ({iaResultado.acertos.length})
                      </div>
                      <ul style={{ margin: 0, paddingLeft: 18, color: t.text, fontSize: 12.5, lineHeight: 1.6 }}>
                        {iaResultado.acertos.map((a, i) => <li key={i}>{a}</li>)}
                      </ul>
                    </div>
                  )}
                </div>
              ...
              {iaResultado.componentes?.length > 0 && (
                <div>
                  <SubTitle color={t.purple}>🔬 Componentes (avaliação da IA)</SubTitle>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 8 }}>
                    {iaResultado.componentes.map((comp, i) => {
                      const cor =
                        comp.status === "correto" ? t.green :
                        comp.status === "ausente" ? t.textFaint :
                        comp.status === "incorreto" ? t.red :
                        comp.status === "incerto" ? t.purple :
                        t.textMuted;
                      return (
                        <div
                          key={i}
                        ...
                        >
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4, gap: 6 }}>
                            <span style={{ fontWeight: 700, color: t.text, fontSize: 12 }}>{comp.nome || "—"}</span>
                            <span
                              style={{
                                padding: "1px 6px",
                                background: cor + "22",
              ...
              {/* Sugestão corrigida pela IA */}
              {iaResultado.sugestaoCorrigida && (
                <div
                  style={{
                    padding: "12px 14px",
                    background: t.greenSoft,
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, gap: 8 }}>
                    <div style={{ fontSize: 11, color: t.green, fontWeight: 800, letterSpacing: "0.05em" }}>
                      ✨ SUGESTÃO CORRIGIDA (IA)
                    </div>
                    <button
                      onClick={copiarSugestaoIA}
                      style={{
                        padding: "4px 10px",
                        background: iaCopiado ? t.green : "transparent",
                        color: iaCopiado ? "#000" : t.green,
                    ...
                    >
                      {iaCopiado ? "✅ Copiado" : "📋 Copiar sugestão"}
                    </button>
                  </div>
                  <div
                    style={{
              ...
              {/* Explicação didática */}
              {iaResultado.explicacaoDidatica && (
                <div
                  style={{
                    padding: "12px 14px",
                    background: t.blueSoft,
                ...
                >
                  <div style={{ fontSize: 11, color: t.blue, fontWeight: 800, marginBottom: 6, letterSpacing: "0.05em" }}>
                    👨‍🏫 EXPLICAÇÃO DIDÁTICA
                  </div>
                  <div style={{ color: t.text, fontSize: 13, lineHeight: 1.65, whiteSpace: "pre-wrap" }}>
                    {iaResultado.explicacaoDidatica}
                  </div>
                </div>
              )}
              ...
              {iaResultado.limitesDaAnalise && (
                ...
                >
                  <b style={{ color: t.text }}>Limites desta análise:</b> {iaResultado.limitesDaAnalise}
                </div>
              )}

              {/* Base da análise — campo fixado pelo servidor, não pelo modelo */}
              {iaResultado.baseDaAnalise && (
                <div
                  style={{
                    padding: "8px 12px",
                    background: t.bg,
                ...
                >
                  📎 base da análise: {iaResultado.baseDaAnalise}
                </div>
              )}
            </div>
          )}
...
export default function App() {
  ...
  const render = () => {
    switch (module) {
      ...
      default: return <ModuleIntro setModule={setModule} />;
    }
  };
